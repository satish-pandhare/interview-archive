...
export const JoinGroupModal = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      await fetch(`/api/groups/invite/${inviteCode}`, {
        method: "PATCH",
      });
      ...
      console.error("[PATCH_GROUP] Error joining group:", error);
    }
  };
    ...
    <Dialog open={isModalOpen} onOpenChange={handleClose}>
      <DialogContent className="p-0 overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          <DialogTitle className="sr-only">Join a group</DialogTitle>
          <DialogDescription className="text-center">
            Join a group by entering the invite code below.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
